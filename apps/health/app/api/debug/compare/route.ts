...
import { findItem, resolveVersion, LookupError } from "@/lib/items";
import { macrosOf } from "@/lib/log";
import { agrees, disagreements, type Macros } from "@/lib/macros";
...
export async function POST(request: NextRequest) {
  ...
  if (!name) return NextResponse.json({ error: "Name a food to compare." }, { status: 400 });

  const onDate = typeof body.date === "string" && /^\d{4}-\d{2}-\d{2}$/.test(body.date)
    ? body.date
  ...
  const bothRan = !haiku.error && !sonnet.error;
  const agreed = bothRan && agrees(haiku.macros, sonnet.macros);
  const differs = bothRan ? disagreements(haiku.macros, sonnet.macros) : [];
  ...
  const { data, error } = await getServiceClient()
    ...
    .insert({
      ...
      haiku: haiku.error ? { error: haiku.error } : { ...haiku.macros, source_url: haiku.source_url, note: haiku.note },
      sonnet: sonnet.error ? { error: sonnet.error } : { ...sonnet.macros, source_url: sonnet.source_url, note: sonnet.note },
      agreed,
    })
    .select("id")
    .single();
