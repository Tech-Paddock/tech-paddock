...
2. **Pick how the login lockout gets fixed — the obvious fix is the wrong shape.** The counter is a
...
3. **The hub's glance has lost its only source — live now, not pending.** *Owner: TechPad Gen.*
...
4. **Delete the Feed tab and rename Board to Pit Wall.** *Owner: TechPad Gen.* Settled 2026-09-18 and
...
5. **Build the `On track` stage.** *Owner: TD.* Agreed 2026-09-17: a fourth phrase and a sixth
...
7. **`shared.contacts` needs its other owner named.** *Owner: Message Editor.* Deliberately shared
   between the editor and the tracker; one of the two is going away. *LoE: minutes.*
8. **Two corrections in charters I may not edit** — re-measured 2026-09-19, and it was three.
...
9. **`Paper.tsx` throws a hydration error on every load.** *Owner: TechPad Gen.* `today()` renders
   the date with a comma on the server and without it in the browser. *LoE: minutes.*
10. **Write the surface guide — site and app.** *Owner: TD.* Joel, 2026-09-18: most tools are
...
11. **No app sets `nosniff` or `Referrer-Policy`, and the hub sends no `frame-ancestors`.**
...
13. **`tp-tracker` stays paused.** Tabled 2026-09-18, paused the same day — verified, `live: false`.
   **Paused is reversible and deleted is not**, so this is a safe place to leave it indefinitely.
   Un-parking means deleting it and its DNS record — no undo, and needs item 3 first. *LoE: minutes.*
