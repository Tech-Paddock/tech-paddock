...
2. **Two tokens on `tp-home`, then redeploy.** `GITHUB_TOKEN` fine-grained, this repo, read-only:
   **Contents, Metadata, Pull requests** — measured against `lib/pitwall.ts`, which calls `/pulls`
   and `/branches` and nothing else. `VERCEL_TOKEN` scoped to the `tech-paddock` team, with an
   expiry; a Vercel token carries the account's permissions within its scope, so scope and expiry
   are the whole mitigation and there is no read-only switch to hunt for.
...
5. **Stand up the health tracker.** *Owner: TD.* Name settled 2026-09-17, guardrails approved, the
   plan is `.claude/HEALTH-PLAN.md` and the gate's own reading is issue #98. **Nothing is blocked on
   Joel any more** — execute `.claude/agents/STANDUP.md` from step 3 rather than re-solutioning what
   is agreed. What still needs him is inside that protocol: the Vercel project and the DNS record
   have no undo and no agent may create them. *LoE: a session.*
...
9. **`CRON_SECRET` and the Microsoft Graph integration.** Parked 2026-09-15.
